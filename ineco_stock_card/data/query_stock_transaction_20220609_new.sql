...
SELECT min(ail.id) as id,
            ...
            round(sum(ail.price_subtotal)/sum(ail.quantity), 2) AS price,
                CASE
                    ...
                    ELSE round(sum(ail.quantity) / 1::numeric, 4)
                END AS quantity,
                CASE
		  ...
		  group by default_code, ai.date_invoice, ai.type, ai.number, ai.reference, ai.partner_id, ai.id
	   UNION
         SELECT sm.id,
            pp.default_code AS stkcod,
