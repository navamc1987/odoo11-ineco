 ...
 SELECT raw.id,
   ...
   FROM ( SELECT ail.id,
            ...
            ail.price_unit AS price,
                CASE
                    WHEN ai.type::text = ANY (ARRAY['out_invoice'::character varying::text, 'in_refund'::character varying::text]) THEN round((- ail.quantity) / 1, 3)
                    ELSE round(ail.quantity / 1, 3)
                END AS quantity,
                CASE
         ...
         SELECT sm.id,
            ...
            COALESCE(( SELECT ir_property.value_float
                ...
                CASE
                    ...
                    ELSE sm.product_qty
                END AS quantity,
            case when sp.origin is not null then sp.origin else sp.name end AS docnum,
        ...
        UNION
         SELECT sm.id,
            pp.default_code AS stkcod,
			case when sm.inventory_id is not null then sm.date::date else (select min(scheduled_date)::date from stock_picking sp where sp.id = sm.picking_id) end as docdat,
            --sm.date_expected::date AS docdat,
            ...
            round(
                CASE
                    ...
                    ELSE sm.product_qty
                END, 3) AS quantity,
            (select name from stock_picking where id = sm.picking_id) AS docnum,
            NULL::integer AS partner_id,
            NULL::integer AS invoice_id
           FROM stock_move sm
             LEFT JOIN stock_location sl ON sl.id = sm.location_dest_id
