 ...
 SELECT raw.id,
   ...
   FROM ( SELECT ail.id,
            ...
            ail.price_unit AS price,
                CASE
                    WHEN ai.type::text = ANY (ARRAY['out_invoice'::character varying::text, 'in_refund'::character varying::text]) THEN round((- ail.quantity) / 1::numeric, 2)
                    ELSE round(ail.quantity / 1::numeric, 2)
                END AS quantity,
                CASE
