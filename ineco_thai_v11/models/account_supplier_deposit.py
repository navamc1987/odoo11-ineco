...
class PurchaseOrder(models.Model):
    ...
    deposit_id = fields.Many2one('ineco.supplier.deposit', string=u'เงินมัดจำ', track_visibility='onchange')
...
class InecoSupplierDeposit(models.Model):
    ...
    po_id = fields.Many2one('purchase.order', string=u'PO', track_visibility='onchange',copy=False)
    ...
    other_ids = fields.One2many('ineco.supplier.deposit.other', 'payment_id', string=u'อื่นๆ')
    ...
    journal_id = fields.Many2one('account.journal', string=u'สมุดรายวันจ่าย', required=True,track_visibility='onchange')
    ...
    wht_ids = fields.One2many('ineco.wht', 'supplier_deposit_id', string=u'ภาษีหัก ณ ที่จ่าย')

    move_id = fields.Many2one('account.move', string=u'สมุดรายวัน', index=True,track_visibility='onchange')
    ...
    supplier_deposit_pay_ids = fields.One2many('ineco.supplier.payment.deposit', 'name', string=u'ประวัติการตัดตั้งหนี้')
    ...
    def button_post(self):
        ...
        vat_sale_account_id = int(params.get_param('ineco_thai_v11.vat_purchase_tax_break_account_id', default=False)) or False,
        if self.amount_vat:
            move_data_vals = {
                'partner_id': False,
                'invoice_id': False,
        ...
        unearned_income_account_id = int(params.get_param('ineco_thai_v11.unearned_expense_account_id', default=False)) or False,
        ...
        if self.amount_cash:
            ...
            iml.append((0, 0, move_data_vals))
        cheque_sale_account_id=int(params.get_param('ineco_thai_v11.cheque_purchase_account_id', default=False)) or False,
        if self.amount_cheque:
            ...
            iml.append((0, 0, move_data_vals))
        cash_discount_account_id=int(params.get_param('ineco_thai_v11.cash_income_account_id', default=False)) or False,
